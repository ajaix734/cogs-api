...
FROM 
	...
	JOIN RT_DATA_ITEM ITM ON SDTL.ITEMID = ITM.ID
...
--	AND (SALES.WORKORDERSTATUS = 'Goods Delivered' OR SALES.WORKORDERSTATUS IS NULL)
	...
	AND UNIT.ACCOUNTUNIT NOT IN ('NAB', 'ERC', 'OHC')
...
FROM 
	...
	JOIN RT_DATA_CORE RDC ON RDC.ID=UNIT.ID
	JOIN RT_ORGANIZATION_CORE ROC ON ROC.ID=RDC.TENANTID
WHERE CRE.CREATEDTIME BETWEEN concat(curdate(),' ','00:00:00') AND concat(curdate(),' ','23:59:59')
